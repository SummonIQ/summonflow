...
export class SummonStream extends EventDispatcher {
  ...
  static fromConfig(config: ConnectionConfig): SummonStream {
    return new SummonStream(config.key, config.options);
  }
...
function createChannel(name: string, client: SummonStream): Channel {
...
function buildConnectionUrl(key: string, options: ConnectionOptions): string {
  ...
  const host = options.wsHost ?? (options.cluster ? `ws-${options.cluster}.summon-stream.local` : "127.0.0.1");
  ...
  url.searchParams.set("client", "summon-stream");
