...
const MANAGED_PLATFORM_HOST = "realtime.summonflow.com";
...
function resolveFetch(options: ConnectionOptions): typeof fetch {
  const bindFetch = (fetchImpl: typeof fetch): typeof fetch =>
    ((...args: Parameters<typeof fetch>) => fetchImpl.call(globalThis, ...args)) as typeof fetch;

  if (options.fetch) {
    return bindFetch(options.fetch);
  }
...
function buildConnectionUrl(key: string, options: ConnectionOptions): string {
  ...
  const host =
    options.wsHost ??
    (options.cluster
      ? `ws-${options.cluster}.summon-flow.local`
      : forceTLS
        ? MANAGED_PLATFORM_HOST
        : "127.0.0.1");
