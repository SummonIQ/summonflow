...
describe("SummonFlow", () => {
  ...
  it("defaults TLS connections to the managed platform host when wsHost is omitted", () => {
    const capturedUrls: string[] = [];

    class InspectableWebSocket extends EventTarget {
      static readonly CONNECTING = 0;
      static readonly OPEN = 1;
      static readonly CLOSING = 2;
      static readonly CLOSED = 3;

      readonly CONNECTING = InspectableWebSocket.CONNECTING;
      readonly OPEN = InspectableWebSocket.OPEN;
      readonly CLOSING = InspectableWebSocket.CLOSING;
      readonly CLOSED = InspectableWebSocket.CLOSED;
    ...
    const client = new SummonFlow("app-key", {
      forceTLS: true,
      fetch,
      WebSocket: InspectableWebSocket as unknown as typeof globalThis.WebSocket,
    });

    expect(capturedUrls).toEqual([
      "wss://realtime.summonflow.com/app/app-key?protocol=7&client=summon-flow&version=0.1.0&flash=false",
    ]);
