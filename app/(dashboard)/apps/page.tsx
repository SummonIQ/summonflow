...
import { CirclePlus, Terminal, Trash2, Loader2, ArrowRight, Activity, Shield } from "lucide-react";
...
export default function AppsPage() {
  ...
  const [isBasicLimitReached, setIsBasicLimitReached] = useState(false);
  ...
  async function fetchApps() {
    ...
    if (appsRes.ok) {
      const nextApps = await appsRes.json();
      setApps(nextApps);
      if (billingRes.ok) {
        const billing = await billingRes.json() as BillingResponse;
        const paidPlan = billing.subscription
          && ["active", "trialing"].includes(billing.subscription.status)
          && ["pro", "enterprise"].includes(billing.subscription.plan);
        setIsBasicLimitReached(!paidPlan && nextApps.length >= 1);
      }
    }
    setLoading(false);
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (res.ok) {
      const app = await res.json().catch(() => null);
      trackProductEvent("application_created", {
        appId: app?.id,
        appName: app?.name ?? newName.trim(),
      });
      setNewName("");
      setShowCreate(false);
      await fetchApps();
      ...
      setCreateError(body?.error ?? "Unable to create application.");
      trackProductEvent("application_create_failed", { reason: body?.error ?? "request_failed" });
    }
    setCreating(false);
  }
  ...
  async function handleDelete(e: React.MouseEvent, appId: string) {
    ...
    if (res.ok) {
      trackProductEvent("application_deleted", { appId, appName: app?.name });
      setApps((prev) => prev.filter((a) => a.id !== appId));
    } else {
      const body = await res.json().catch(() => null);
      trackProductEvent("application_delete_failed", { appId, appName: app?.name, reason: body?.error ?? "request_failed" });
    }
  }
  ...
  return (
    <main className="space-y-5">
      <header className="flex flex-col gap-3 px-1 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-white">Applications</h1>
        ...
        <button
          onClick={() => {
            if (isBasicLimitReached) {
              trackProductEvent("application_create_blocked", { reason: "basic_application_limit", appCount: apps.length });
              return;
            }
            setCreateError(null);
            setShowCreate(true);
            trackProductEvent("application_create_modal_opened", { appCount: apps.length });
          }}
          disabled={isBasicLimitReached}
          className="button-default inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-zinc-950 transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      {isBasicLimitReached && (
        <div className="rounded-lg border border-zinc-800 bg-zinc-950/50 px-4 py-3 text-sm text-zinc-400">
          Basic plan is limited to 1 application. Upgrade to Pro to create more applications.
        </div>
      )}
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => {
          setShowCreate(false);
          setNewName("");
          trackProductEvent("application_create_modal_closed", { source: "backdrop" });
        }}>
          <form
          ...
          >
            ...
            {createError && (
              <div className="rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2 text-xs font-medium text-red-400">
                {createError}
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3">
              <button type="button" onClick={() => {
                setShowCreate(false);
                setNewName("");
                trackProductEvent("application_create_modal_closed", { source: "cancel_button" });
              }} className="rounded-lg px-4 py-2.5 text-sm text-zinc-400 hover:text-white transition">
                Cancel
              </button>
              <button type="submit" disabled={creating || !newName.trim()} className="button-default rounded-lg px-5 py-2.5 text-sm font-semibold text-zinc-950 transition disabled:opacity-50">
        ...
        <div className="overflow-hidden rounded-xl border border-zinc-800/50">
          {apps.map((app, i) => (
            <Link
              key={app.id}
              href={`/apps/${app.id}`}
              onClick={() => trackProductEvent("application_opened", { appId: app.id, appName: app.name, source: "applications_list" })}
            ...
            >
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-px text-[11px] text-zinc-500">
