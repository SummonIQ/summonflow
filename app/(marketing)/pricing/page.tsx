...
export const metadata: Metadata = {
  title: "Pricing",
  description:
    "SummonFlow pricing — free to self-host forever, or use the managed platform starting at $0/mo. Production-grade realtime infrastructure.",
  alternates: { canonical: "https://summonflow.com/pricing" },
};
      ...
      <div className="mx-auto max-w-[76rem]">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {plans.map((plan) => (
            <section
              key={plan.name}
              className={`relative flex flex-col justify-between rounded-[2rem] border p-8 ${
                plan.highlight
                  ? "mesh-card-no-clip border-[var(--accent)] ring-1 ring-[var(--accent)] ring-offset-4 ring-offset-[var(--background)]"
                  : "mesh-card border-[var(--line)]"
              }`}
            >
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 z-10 -translate-x-1/2 rounded-full bg-[var(--accent)] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white">
                  Most Popular
                </div>
              )}
