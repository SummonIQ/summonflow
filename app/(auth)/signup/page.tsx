...
import { Activity, ArrowRight, Loader2, Github, ShieldCheck, Radio } from "lucide-react";
...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    trackProductEvent("signup_attempted", { method: "email", emailDomain: email.split("@")[1] ?? null });

    if (!firstName || !lastName) {
      setError("First and last name are required.");
      trackProductEvent("signup_failed", { method: "email", reason: "missing_name" });
      setLoading(false);
      return;
    }
    ...
    if (password !== confirmPassword) {
      ...
      trackProductEvent("signup_failed", { method: "email", reason: "password_mismatch" });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (result.error) {
        ...
        trackProductEvent("signup_failed", { method: "email", reason: result.error.message ?? "signup_error" });
        setLoading(false);
        return;
      }
      trackProductEvent("signup_succeeded", { method: "email", emailDomain: email.split("@")[1] ?? null });
      router.push("/apps");
    } catch {
      setError("Something went wrong. Please try again.");
      trackProductEvent("signup_failed", { method: "email", reason: "exception" });
      setLoading(false);
    }
  }
  ...
  async function handleGithubSignUp() {
    ...
    trackProductEvent("signup_attempted", { method: "github" });
    try {
      await authClient.signIn.social({
        provider: "github",
        callbackURL: "/apps",
      });
      trackProductEvent("signup_redirect_started", { method: "github" });
    } catch {
      setError("GitHub sign-up failed. Check your provider configuration.");
      trackProductEvent("signup_failed", { method: "github", reason: "exception" });
